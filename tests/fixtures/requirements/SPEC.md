...
- Дерево строится через единый resolver ADR-005: из explicit путей `.harness/manifest.yaml`, а для schema gaps — только из централизованных зарегистрированных derivations от manifest anchors; consumer-level hardcode и filesystem search запрещены.
