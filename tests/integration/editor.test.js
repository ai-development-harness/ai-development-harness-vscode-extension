...
suite('STEP editor (STEP-007)', () => {
  test('обрабатывает открытый до activation STEP в custom manifest taskDirectory', async () => {
    const ext = vscode.extensions.getExtension('ai-development-harness.harness-navigator');
    const root = vscode.workspace.workspaceFolders[0].uri.fsPath;
    const manifestPath = path.join(root, '.project', 'manifest.yaml');
    ...
    try {
      ...
      // Custom path не покрывается static filenamePatterns, поэтому именно
      // initial sweep при activation обязан перевести уже открытый Markdown.
