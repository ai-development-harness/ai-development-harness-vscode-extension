...
STEP-001, STEP-002, STEP-003, STEP-004 `Выполнено`. `STEP-006` и `STEP-007` зависели только от `STEP-003` — полностью разблокированы, доступны для `PLAN` (могут вестись параллельно). `STEP-005` зависел от STEP-001, STEP-003, STEP-004 — все три выполнены, теперь полностью разблокирован. `STEP-008`/`STEP-009` по-прежнему ждут `STEP-005`.
