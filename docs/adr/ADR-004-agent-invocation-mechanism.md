...
5. **Промпт и весь собранный контекст передаются через stdin**, вызов — через `child_process.spawn` с argv-массивом, **не** через shell-строку. Причина — реально воспроизведённый баг: variadic-опция `claude --allowedTools <tools...>` при ином порядке аргументов «съедает» следующий позиционный prompt-аргумент; тот же класс проблем (quoting/escaping/argv-injection) в общем случае непредсказуем, когда контекст содержит текст из репозитория пользователя. **Эмпирически подтверждено `FIX STEP-001`** (не только по документации): живой вызов `echo "<prompt>" | codex exec --json ...` дошёл до `turn.started` с содержимым из stdin (`stderr: "Reading prompt from stdin..."`); `echo "<prompt>" | claude -p --output-format json ...` — полный happy path через stdin (`evidence/claude-print-stdin-stdout.json`). Stdin-передача снимает проблему целиком для обоих CLI, подтверждено, не предположено.
6. **Cancel** — `child_process` kill (`SIGTERM`) выбранного процесса. **Эмпирически подтверждено для Claude** (`FIX STEP-001`, F-003; полная расшифровка — `evidence/claude-cancel-test.txt`): дочерний процесс завершался в пределах 1с после `SIGTERM` (exit 143), без orphan-процессов, без порчи repository (`git status` идентичен до/после). Важно: при `--output-format json` (не `stream-json`) отменённый вызов не оставляет НИКАКОГО частичного результата в stdout — Agent integration layer обязан трактовать cancel как «результата нет», не пытаться восстановить partial JSON. Для Codex cancel-поведение не тестировалось отдельно (см. Consequences) — сессия при `--ephemeral` не оставляет файлов независимо от способа завершения. `session_id` из финального JSON Claude можно использовать для `--resume` в будущем, но не обязателен для MVP cancel-семантики.
...
Плюсы: реально подтверждённый, а не гипотетический механизм; переиспользует существующую аутентификацию пользователя; не увеличивает вес bundle расширения (Вариант D vs B); безопасен по умолчанию (read-only/plan permission для non-mutating команд); stdin-передача и cancel-семантика (Claude) подтверждены живыми вызовами, не только документацией. Минусы: два разных CLI с разными флагами/форматом JSON — Agent integration layer обязан инкапсулировать это различие за общим интерфейсом (пункт 4 Decision), а не протекать наружу в командный слой (STEP-005); happy-path Codex (turn.completed с реальным результатом) не подтверждён эмпирически в рамках этого STEP из-за исчерпанной квоты аккаунта — риск зафиксирован явно, не скрыт, переподтвердить перед/во время STEP-009; cancel-поведение подтверждено только для Claude, не для Codex — при реализации STEP-009 повторить cancel-тест на Codex, когда квота позволит получить содержательное (не мгновенно-провальное) окно выполнения.

## Security implications

Никогда не использовать `--dangerously-bypass-approvals-and-sandbox`/`--dangerously-skip-permissions`. Промпт/контекст — только через stdin (argv injection risk снят и эмпирически подтверждён, см. Decision п.5). Sandbox/permission mode — минимально необходимый для типа команды (read-only для non-mutating, ограниченный write-scope для mutating, никогда unrestricted).
