...
Плагин читает/пишет только файлы внутри workspace, попадающие под пути из `.project/manifest.yaml`, либо под собственный фиксированный путь настроек `.project/harness-config.json` (STEP-004: не Harness-протокольный артефакт, не подпадает под ADR-001 — единолично создаётся и читается самим расширением, а не резолвится через манифест). Не выполняет произвольный shell помимо git-операций через `simple-git` и вызова агента с фиксированными, не пользовательски-инъецируемыми аргументами. Не отправляет телеметрию, не обращается к внешним сервисам кроме выбранного механизма вызова агента.
